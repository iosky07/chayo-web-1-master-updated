...
{{--        {{dd($action)}}--}}
        ...
        <x-select :options="$optionPacketTags" :selected="$packetTags" title="Pilih Paket Wifi" model="customer.packet_tag_id"/>

        <x-input type="text" title="Longitude" model="customer.longitude"/>

        <x-input type="text" title="Latitude" model="customer.latitude"/>

        <x-date type="datetimepicker" title="Tanggal Registrasi" model="customer.registration_date"/>
...
<script>
    document.addEventListener('livewire:load', function () {
        window.livewire.on('redirect', () => {
            setTimeout(function () {
                window.location.href = "{{route('admin.customer.index')}}"; //will redirect to your blog page (an ex: blog.html)
